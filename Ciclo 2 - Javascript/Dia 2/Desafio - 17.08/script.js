...
/*
	...
	como é um prompt, em regra será um string
	caso tivesse "parseFloat()" ou "parseInt()" antes do prompt ela em regra será um número ou NaN
	...
	todavia, se o usuário cancelar, o prompt irá retornar null
		então o loop while abaixo serve para prevenir isso, pausando o programa até que o usuário
		digite algo válido, ou até mesmo aperte "Ok" sem digitar nada
*/
var numeroString = prompt(nome+", digite um numero para comparar com 'n'");

while(numeroString===null){
	alert("Espera "+nome+"!\nPor que você cancelou?")
	var numeroString = prompt("Vamos tentar novamente "+nome+"!\nDigite um numero para comparar com 'n'");
}

alert("O Javascript entendeu esse prompt como se fosse um "+typeof(numeroString)+".");
alert("Mas não se preocupe, esse "+typeof(numeroString)+" será convertido em número!\nEspero que você tenha digitado um número mesmo em!");

/*
	antes de converter com o parseFloat, usa-se replace na vírgula para um ponto
		as linguagens de código são baseadas no inglês, e geralmente, nos países que falam a língua, eles
		utilizam o ponto ao invés da vírgula para denotar as casas decimais
	entretanto, aqui no brasil nós utilizamos a vírgula, então esse replace é bem relevante para
		posteriormente verificar se o número é inteiro ou não
*/
var numeroString = numeroString.replace(',', '.');
...
/*
	...
	o exatamente diferente (!==) funciona neste caso pois o valor NaN tem uma propriedade interessante
		ele não é igual(==) ou identico(===) a nenhum valor, nem mesmo ele mesmo
	...
	o if(numeroString!=null) foi incluído pois caso contrário aconteceriam alguns bugs caso o usuário cancelasse
		como cancelar retorna (null), isso causava alguns problemas com a operação (replace)
*/
...
/*
	uma preocupação de pedir ao usuário escrever uma palavra(sequencia de caracteres, ou STRING)
		...
		alguns escrevem tudo minúsculo, uns colocam a primeira letra maíuscula, e outros até com CAPS LOCK
	...
	após isso, comparamos o que o usuário digitou (var x) com as variáveis através da função (localeCompare)
...
//aqui temos os cálculos a serem realizados com n mais o quadrado do número escolhido
//pelo usuário
...
//por fim, imprimem-se os resultados dos cálculos realizados anteriormente
